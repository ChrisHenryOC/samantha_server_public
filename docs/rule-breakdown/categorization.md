...
| IHC-002 | `Equals('event.all_slides_complete', true)` | Equals | Safety relies on the **no-co-occurrence contract** (see decision-gate.md § "Dispatch contract"): `ihc_qc` events carry either `all_slides_complete` OR `staining_failure`/`tissue_available`, never both — verified across the upstream POC repo's `scenarios/rule_coverage/`. If a future event source ever emits both keys on one event, IHC-002 needs `BooleanAnd(..., Not(Equals('event.staining_failure', true)))`. |
