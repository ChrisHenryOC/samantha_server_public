"""GH-196 / GH-363: gen_ai.completion and gen_ai.prompt on OTel spans.

Local-LLM + self-hosted Langfuse topology allows LLM completions on spans for
operator diagnostics. This module drives all four GH-196 slices through
red-green-refactor:
...
Slice 3 — gen_ai.prompt default-on (GH-363); disable with falsy SAMANTHA_STAMP_PROMPT
"""
...
def test_slice1_gen_ai_completion_is_on_allowlist() -> None:
    """GH-196 Slice 1: gen_ai.completion must be in _ALLOWED_ATTRIBUTES."""
    from samantha_server.observability.otel import _ALLOWED_ATTRIBUTES

    assert "gen_ai.completion" in _ALLOWED_ATTRIBUTES, (
        "GH-196: gen_ai.completion must be allowlisted for lab-host Langfuse diagnostics"
    )


def test_slice1_set_span_attribute_accepts_gen_ai_completion() -> None:
    """GH-196 Slice 1: set_span_attribute does NOT raise for gen_ai.completion."""
...
def test_slice2_llm_complete_with_span_stamps_completion(
    ...
    """GH-196 Slice 2: llm_complete_with_span stamps gen_ai.completion = response.text."""
    from opentelemetry import trace as otel_trace
    ...
    assert attrs.get("gen_ai.completion") == completion_text, (
        f"GH-196: gen_ai.completion must equal response.text; attrs={attrs}"
    )
...
def test_slice2_llm_complete_json_with_span_stamps_completion(
    ...
    """GH-196 Slice 2: llm_complete_json_with_span stamps gen_ai.completion = response.text."""
...
def test_slice3_gen_ai_prompt_is_on_allowlist() -> None:
    """GH-196 Slice 3: gen_ai.prompt must be in _ALLOWED_ATTRIBUTES."""
    from samantha_server.observability.otel import _ALLOWED_ATTRIBUTES

    assert "gen_ai.prompt" in _ALLOWED_ATTRIBUTES, (
        "GH-196: gen_ai.prompt must be allowlisted for prompt diagnostics"
    )
...
def test_slice3_prompt_stamped_by_default(
    ...
    """GH-363: with SAMANTHA_STAMP_PROMPT unset, gen_ai.prompt is present (default-on)."""
    ...
    assert attrs.get("gen_ai.prompt") == prompt_text, (
        "GH-363: gen_ai.prompt must be present when SAMANTHA_STAMP_PROMPT is unset (default-on)"
    )
...
def test_slice3_prompt_stamped_when_env_on(
    ...
    """GH-196 Slice 3: with SAMANTHA_STAMP_PROMPT=1, gen_ai.prompt is stamped."""
    ...
    attrs = dict(child.attributes or {})
    assert attrs.get("gen_ai.prompt") == prompt_text, (
        f"GH-196: gen_ai.prompt must equal prompt when SAMANTHA_STAMP_PROMPT=1; attrs={attrs}"
    )
...
def test_slice3_json_prompt_stamped_as_canonical_json(
    ...
    """GH-196 Slice 3: SAMANTHA_STAMP_PROMPT=1, json wrapper stamps prompt as json.dumps."""
    ...
    assert attrs.get("gen_ai.prompt") == expected, (
        f"GH-196: gen_ai.prompt must equal json.dumps(messages) "
...
def test_pr207_stamp_prompt_env_falsy_values(
    ...
    """GH-363: explicit falsy values (0/false/no/off, case-insensitive) disable prompt stamping.
...
# GH-229's diagnosis: the issue body claimed "input prompt was never
...
def test_prompt_stamped_on_error_path_text_endpoint(
    ...
    # PR #269 review #1: pin the error status + recorded-exception contract.
...
def test_prompt_not_stamped_on_error_path_when_env_off_text_endpoint(
    ...
    """GH-363: explicit SAMANTHA_STAMP_PROMPT=0 suppresses gen_ai.prompt on error path.
...
def test_prompt_not_stamped_on_error_path_when_env_off_json_endpoint(
    ...
    """GH-363: explicit SAMANTHA_STAMP_PROMPT=0 suppresses gen_ai.prompt on error path (JSON)."""
    import samantha_server.config as cfg
    from samantha_server.observability.otel import (
        LLM_CHILD_SPAN_NAME,
        llm_complete_json_with_span,
